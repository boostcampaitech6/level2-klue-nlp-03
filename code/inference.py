...
def main(args):
  ...
  state_dict = torch.load(os.path.join(f'./best_model', 'roberta-large_14.bin'))
  ...
  output.to_csv('./prediction/submission_roberta-large_14.csv', index=False) # 최종적으로 완성된 예측한 라벨 csv 파일 형태로 저장.
  #### 필수!! ##############################################
  print('---- Finish! ----')
